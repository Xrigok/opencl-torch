...
using c10::DeviceType;
    ...
    Tensor & gelu_(Tensor & self,c10::string_view approximate){
        ...
        dlprim::Tensor X = todp(self);
        dlprim::ExecutionContext q = getExecutionContext(self);
        dlprim::core::activation_forward(X,X,dlprim::StandardActivations::gelu,q);
    ...
    Tensor & mycomp_out(const Tensor & self, const Scalar & other, Tensor & out,std::string const &op)
    {
        GUARD;
        if(out.numel() == 0){
            return out;
        ...
        dlprim::core::mypointwise_operation_broadcast(x0,y0,w0,"y0 = x0 " + op + " w ? 1 : 0;", getExecutionContext(self));
    ...
    {
        return mycomp_out(self,other,out,"<=");
    ...
    Tensor & upsample_nearest2d_out(const Tensor & self,c10::SymIntArrayRef output_size, c10::optional<double> scales_h, c10::optional<double> scales_w,Tensor & out){
        GUARD;
        Tensor self_c=self.contiguous();
        dlprim::Tensor X=todp(self_c);
        dlprim::Tensor Y=todp(out);
        ...
        if(scales_h&&scales_w){
            ...
            s_h=oshape[oshape.size()-2]/ishape[ishape.size()-2];
            s_w=oshape[oshape.size()-1]/ishape[ishape.size()-1];
        }
        auto un=dlprim::core::UpsampleNearest2dFwd::create(ctx,ishape,oshape,s_h,s_w,X.dtype());
        un->enqueue(X,Y,q); 
        sync_if_needed(self.device());
        return out;
    ...
    Tensor & pow_Tensor_Scalar_out(Tensor const& self,c10::Scalar const& exponent,Tensor &out){
        GUARD;
        Tensor self_c=self.contiguous();
        dlprim::Tensor X=todp(self_c);
        dlprim::Tensor Y=todp(out);
        dlprim::ExecutionContext q=getExecutionContext(self);
        dlprim::Context ctx(q);
        dlprim::core::pointwise_operation({X},{Y},{exponent.to<double>()},"y0 = pow(x0,w0);",q);
...
TORCH_LIBRARY_IMPL(aten, PrivateUse1, m) {
      ...
      m.impl("aten::gelu_",&ptdlprim::gelu_);
      ...
      m.impl("aten::pow.Tensor_Scalar_out",&ptdlprim::pow_Tensor_Scalar_out);
}
