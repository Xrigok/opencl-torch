...
using c10::DeviceType;
    ...
    ::std::tuple<Tensor,Tensor,Tensor> native_layer_norm(const Tensor & input, c10::ArrayRef<long> normalized_shape,const c10::optional<Tensor> & weight, const c10::optional<Tensor> & bias, double eps)
    {
        GUARD;
        bool weight_present = weight && weight->numel()>0; 
        bool bias_present = bias && bias->numel()>0; 
        ...
        //check layernorm dim and input shape
        int norm_shape_size=normalized_shape.size();
        auto xshape=X.shape();
        int x_shape_size=xshape.size();
        long ns=1;
        for(int i=0;i<norm_shape_size;++i){
            ns*=normalized_shape[norm_shape_size-i-1];
            TORCH_CHECK(normalized_shape[norm_shape_size-i-1] == xshape[x_shape_size-i-1],"layernorm dim diffirence with input shape, please check")
        }

        auto ln = dlprim::core::LayerNormFwdBwd::create(ctx,X.shape(),ns,X.dtype());
        
        Tensor calc_mean_pt,calc_var_pt;

        if(affine) {
            ln->enqueue_forward_affine(X,Y,gamma,beta,eps,q);
        }
        else {
            ln->enqueue_forward_direct(X,Y,eps,q);
        }
