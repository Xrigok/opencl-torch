...
using c10::DeviceType;
    ...
    ::std::tuple<Tensor&,Tensor&> max_pool2d_with_indices_out(Tensor const &self, IntArrayRef kernel_size, IntArrayRef stride, IntArrayRef padding, IntArrayRef dilation, bool ceil_mode,Tensor & out, Tensor &indices) {
        GUARD;
        Tensor self_c = self.contiguous();
        
        dlprim::Tensor X=todp(self_c);
        dlprim::Tensor Y=todp(out);
        dlprim::Tensor inds=todp(indices);
        ...
        int dila[2] = {int(dilation[0]), int(dilation[1])};

        auto maxpool2dindices = dlprim::core::MaxPooling2DIndicesForward::create(
                        ctx,X.shape(),Y.shape(),
                        ker,pad,strd,dila,
                        X.dtype()
                    );                  
        maxpool2dindices->enqueue(X,Y,inds,q);    
        sync_if_needed(self.device());

        return std::tuple<torch::Tensor&,torch::Tensor&>(out,indices);
    }
